...
public class BuddyWnd extends Widget implements Iterable<BuddyWnd.Buddy> {
    ...
    public BuddyWnd() {
        ...
        addhl(prev.pos("bl").adds(0, 2), sz.x,
                prev = new Button(sbw, "Status").action(() -> {
                    setcmp(statuscmp);
                }),
                new Button(sbw, "Group").action(() -> {
                    setcmp(groupcmp);
                }),
                new Button(sbw, "Name").action(() -> {
                    setcmp(alphacmp);
                })
        );
        ...
        addhl(charpass.pos("bl").adds(0, 5), sz.x,
                prev = new Button(sbw, "Set").action(() -> {
                    setpwd(charpass.text);
                }),
                new Button(sbw, "Clear").action(() -> {
                    setpwd("");
                }),
                new Button(sbw, "Random").action(() -> {
                    setpwd(randpwd());
                })
        );
