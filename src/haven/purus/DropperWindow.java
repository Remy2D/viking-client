...
public class DropperWindow extends BetterWindow {
    ...
    public void refresh() {
        vl.clearItems();
        Config.autodropItems.val.entrySet().stream().sorted((a, b) -> a.getKey().compareTo(b.getKey())).forEach(vi -> {
            vl.addItem(new AutodropItem(vi.getKey(), vi.getValue()));
        });
    }
    ...
    public class DropList extends Widget {
        ...
        public void clearItems() {
            this.children().forEach(w -> {
                if (w instanceof AutodropItem) w.destroy();
            });
            items.clear();
        }
