...
public class DangerManagerWindow extends BetterWindow {
    ...
    public void refresh() {
        vl.clearItems();
        Config.animalRads.val.entrySet().stream().sorted((a, b) -> a.getKey().compareTo(b.getKey())).forEach(vi -> {
            vl.addItem(new RadiusItem(vi.getKey(), vi.getValue()));
        });
    }
    ...
    public class RadiusList extends Widget {
        ...
        public void clearItems() {
            this.children().forEach(w -> {
                if (w instanceof RadiusItem) w.destroy();
            });
            items.clear();
        }
