...
public class FlowerManagerWindow extends BetterWindow {
    ...
    public void refresh() {
        vl.clearItems();
        Config.flowerOptOpens.val.entrySet().stream().sorted((a, b) -> a.getKey().compareTo(b.getKey())).forEach(vi -> {
            vl.addItem(new PetalItem(vi.getKey(), vi.getValue()));
        });
    }
    ...
    public class PetalList extends Widget {
        ...
        public void clearItems() {
            this.children().forEach(w -> {
                if (w instanceof PetalItem) w.destroy();
            });
            items.clear();
        }
